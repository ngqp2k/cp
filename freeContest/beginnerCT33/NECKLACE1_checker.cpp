...
long long validate(vector<int> took) 
{
    sort (took.begin(), took.end());
    took.resize(unique(took.begin(), took.end()) - took.begin());
    if (took.size() != k)
        return -1;
    if (took[0] < 0 || took[took.size() - 1] >= n) 
        return -1;
    long long ret = a[took[0]];
    for (int i=1; i<took.size(); ++i)   
    {
        ret += a[took[i]];
        if (took[i] - 1 == took[i - 1])
            return -1;
    }
    if (took[0] == 0 && took[took.size() - 1] == (n - 1)) 
        return -1;
    return ret;
}
...
int main(int argc, char * argv[]) 
{
    ...
    long long output = ouf.readLong(); ouf.skipBlanks();
    vector<int> trace;
    while ((!ouf.eoln()) && (trace.size() <= k)) 
    {
        trace.push_back(ouf.readInt()); ouf.skipBlanks();
    }
    long long valid = validate(trace);
    int score = 0;
    if (answer == output) 
        score += 1;
    if (valid == answer) 
        score += 1;
    if (valid == output && output < answer) 
    {
        score = 2;
        quitf(_ok, "better than jury");
    }
    else 
    {
        switch (score)
        ...
        case 0:
            ...
            break;
        case 1: 
            quitp(0.5L, "incorrect trace");
            break;
        case 2: 
            quitf(_ok, "ok");
            break;
        default:
            quitf(_fail, "how did we get there?");
            break;
        }
    }
}